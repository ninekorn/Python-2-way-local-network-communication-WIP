...
somesentfilecounter = 0
somesentfilecountermax = 10

somesentfilemaincounter = 0
#https://stackoverflow.com/questions/55024440/brokenpipeerror-errno-32-broken-pipe-error-when-running-gans
    ...
    for x in y:
        ...
        if someswtc == -1:
            if somereconnectcounter >= somereconnectcountermax:          
                try:     
                    ...
                    # get the file size
                    serverfilesize = os.path.getsize(filename)
                    
                    someotherservername = str(somesentfilemaincounter) + serverfilename
                    
                    print(someotherservername)
                    # send the filename and filesize
                    s.send(f"{someotherservername}{SEPARATOR}{serverfilesize}".encode())
                    #s.send("PcToPiMsg".encode())
                    somesentfilemaincounter+=1
                    # start sending the file
                    progress = tqdm.tqdm(range(serverfilesize), someotherservername, unit="B", unit_scale=True, unit_divisor=1024)
                    someswtc = 0
                except Exception as ex:
                    print(ex)
                    someswtc = -1
        ...
        if  someswtc == 1:           
            if somelistencounter >= somelistencountermax:
                ...
                if somesentfilecounter >= somesentfilecountermax:
                    print("task to receive 10 files from rasppi to pc is over.")
                    someswtc = - 9
                else:         
                    ...
                    print("[*] Listening ip/port",(hostmain,portmain))
                    ...
                    try:
                        ...
                        somefilename, filesize = received.split(SEPARATOR)
                        # remove absolute path if there is
                        somefilename = os.path.basename(somefilename)
                        # convert to integer
                        filesize = int(filesize)
                        #somefilename = received
                        #filesize = 0
                        
                        
                        # start receiving the file from the socket
                        # and writing to the file stream
                        progress = tqdm.tqdm(range(filesize), somefilename, unit='B', unit_scale=True, unit_divisor=1024)
